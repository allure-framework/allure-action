...
const resolveQualityGateTestResults = async (
  ...
  const resolved = await Promise.all(
    uniqueTestResultIds.map(async (testResultId) => {
      ...
      const fileTest = registryTest ? undefined : await readTestResultFile(testResultId, options.reportDir);
      const merged = {
        id: testResultId,
        name: testResultId,
...
const renderQualityGateEntry = ({
  environment,
  result,
  relatedTestCount,
  tests,
}: QualityGateCommentEntry): string[] => {
  const lines = [
    "<details>",
