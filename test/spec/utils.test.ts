...
describe("utils", () => {
  ...
  describe("generateSummarySectionComments", () => {
    ...
    it("should truncate oversized section comments and append More link", () => {
      ...
      expect(result.body).toContain(
        '<a href="https://example.com/suite-a/?transition=new" target="_blank" rel="noopener noreferrer">More</a>',
      );
    ...
    it("should escape special characters in generated More link", () => {
      ...
      expect(result.body).toContain(
        '<a href="https://example.com/suite?a=1&amp;b=&lt;tag&gt;&amp;c=&quot;quote&quot;&amp;transition=new" target="_blank" rel="noopener noreferrer">More</a>',
      );
    });
  ...
  describe("generateSummaryMarkdownTable", () => {
    ...
    it("should render generation environments once above the table and per-summary resolution counters", () => {
      const summaries = [
        {
          name: "Test Suite 1",
          stats: {
    ...
    it("should escape untrusted environment names", () => {
      const result = generateSummaryMarkdownTable([], {
        environments: ['Chrome | <script>alert("x")</script>'],
      });

      expect(result).toContain(
        "**Environments:** <code>Chrome | &lt;script&gt;alert(&quot;x&quot;)&lt;/script&gt;</code>",
      );
    });
    ...
    it("should escape special characters in report links", () => {
      ...
      expect(result).toContain(
        '<a href="https://example.com/report?tab=&quot;overview&quot;&amp;q=&lt;fast&gt;&amp;x=y&amp;transition=new" target="_blank" rel="noopener noreferrer">1</a>',
      );
      expect(result).toContain(
    ...
    it("should use canonical Allure 3 filter parameters", () => {
      const summaries = [
        {
          name: "Test Suite 1",
          stats: { passed: 1, failed: 0, broken: 0, skipped: 0, unknown: 0 },
          duration: 1000,
          remoteHref: "https://example.com/report/#results",
          newTests: ["new-1"],
          flakyTests: ["flaky-1"],
          retryTests: ["retry-1"],
        },
      ...
      expect(result).toContain('href="https://example.com/report/?transition=new#results"');
      expect(result).toContain('href="https://example.com/report/?flaky=true#results"');
      expect(result).toContain('href="https://example.com/report/?retry=true#results"');
